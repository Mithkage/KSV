...
namespace RTS.Commands.Utilities
{
    ...
    public class RTS_MapCablesClass : IExternalCommand
    {
        ...
        private Result GenerateCsvTemplate(Autodesk.Revit.ApplicationServices.Application application)
        {
            try
            {
                using (var folderDialog = new FolderBrowserDialog())
                {
                    ...
                    if (folderDialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        CustomTaskDialog.Show("RTS_MapCables", $"CSV template file generated successfully at:\n{filePath}");
                        return Result.Succeeded;
                    }
                    else
                    {
                        CustomTaskDialog.Show("RTS_MapCables", "CSV template generation cancelled by user.");
                        return Result.Cancelled;
                    }
                }
            }
            ...
            {
                CustomTaskDialog.Show("RTS_MapCables Error", $"Failed to generate CSV template: {ex.Message}");
        ...
        private Result ProcessAndMapParameters(UIDocument uidoc)
        {
            if (uidoc == null)
            {
                CustomTaskDialog.Show("RTS_MapCables Error", "No active Revit document found.");
                return Result.Failed;
            }
            ...
            using (OpenFileDialog openFileDialog = new OpenFileDialog())
            {
                ...
                {
                    CustomTaskDialog.Show("RTS_MapCables", "CSV file selection cancelled.");
                    return Result.Cancelled;
                }
            }
            ...
            {
                CustomTaskDialog.Show("RTS_MapCables Error", $"Failed to read or parse CSV file: {ex.Message}");
                return Result.Failed;
            }

            if (!csvMappingEntries.Any())
            {
                CustomTaskDialog.Show("RTS_MapCables", "No valid mapping entries found in the selected CSV file (ensure 'Client GUID', 'Client Parameter Name', or 'Notes' column is populated).");
        ...
        private Result MapParametersOnElements(Document doc, List<CsvMappingEntry> mappingEntries)
        {
            ...
            {
                CustomTaskDialog.Show("RTS_MapCables", "No Cable Trays, Cable Tray Fittings, Conduits, or Conduit Fittings found in the project to map parameters.");
                return Result.Succeeded;
            }
            ...
            using (Transaction t = new Transaction(doc, "Map RTS Parameters"))
            {
                try
                {
                    ...
                    // Show a simple CustomTaskDialog confirming completion and report location
                    CustomTaskDialog.Show("RTS_MapCables",
                                    $"Mapping complete.\n\n" +
                ...
                catch (Exception ex)
                {
                    ...
                    t.RollBack();
                    CustomTaskDialog.Show("RTS_MapCables Error", $"Mapping Failed: {ex.Message}\nTransaction rolled back. No changes were applied to the model.");
                    return Result.Failed;
                }
            }
        }
        ...
        private string ExportSystemNotesReportToCsv(List<SystemNotesReportEntry> reportEntries)
        {
            ...
            try
            {
                using (var saveFileDialog = new SaveFileDialog())
                {
                    ...
                    if (saveFileDialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        File.WriteAllText(filePath, csvContent.ToString());
                        // No CustomTaskDialog here, it will be handled by the calling method.
                    }
                    else
                    {
                        // CustomTaskDialog.Show("RTS_MapCables", "System Notes Report export cancelled by user."); // Removed as per request
            ...
            {
                CustomTaskDialog.Show("RTS_MapCables Error", $"Failed to export System Notes Report: {ex.Message}");
