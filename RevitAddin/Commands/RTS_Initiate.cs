...
// Author: Kyle Vorster
//
// Date: June 18, 2024 (Updated July 16, 2025)
//
/*
 * Change Log:
 *
 ...
 * 2025-07-16 | 1.0.6   | Gemini | Modified `ProcessParameters` to prevent setting `AllowVaryBetweenGroups` on parameters
 * bound to the `OST_DetailComponents` category, resolving an `ArgumentException` for certain parameters like 'PC_PowerCAD'.
 * 2025-07-16 | 1.0.7   | Gemini | Aligned `SharedParameterInfo` names for cable/conduit parameters with existing project names
 * (e.g., "Cable_01" instead of "RTS_Cable_01") to resolve name mismatch warnings during execution.
 * 2025-07-16 | 1.0.8   | Gemini | Added the 'RTS_Type' shared parameter and applied it to both 'Detail Items'
 * and 'Generic Annotations' categories as requested.
 * 2025-07-16 | 1.0.9   | Gemini | Resolved `InvalidOperationException` by removing the attempt to bind a parameter to the 'Generic Annotations'
 * category, which is not permitted by the Revit API. The parameter is now only applied to 'Detail Items'.
 * 2025-07-16 | 1.1.0   | Gemini | Refactored command to be context-aware. If run in a family document (Detail Item or Generic Annotation),
 * it now adds 'RTS_Type' as a Type parameter under 'Identity Data'. Project-level logic remains for project documents.
 * 2025-07-16 | 1.2.0   | Gemini | Integrated a WPF UI to allow users to select which parameter groups to initiate. The command now
 * displays a window and processes only the user-selected categories.
 */
...
using RTS.UI; // Add this using statement for the UI
using System;
...
namespace RTS.Commands
{
    ...
    public class RTS_InitiateClass : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            // Check if the command is being run in a Family Document
            if (doc.IsFamilyDocument)
            {
                return AddRtsTypeToFamily(commandData, ref message);
            }
            // Otherwise, run the project-level parameter initiation
            else
            {
                return InitiateProjectParameters(commandData, ref message, elements);
            }
        }

        /// <summary>
        /// Adds the 'RTS_Type' parameter to a compatible family document.
        /// </summary>
        private Result AddRtsTypeToFamily(ExternalCommandData commandData, ref string message)
        {
            UIApplication uiapp = commandData.Application;
            Document doc = uiapp.ActiveUIDocument.Document;
            Application app = uiapp.Application;

            // 1. Check the category of the family. Must be Detail Component or Generic Annotation.
            Family family = doc.OwnerFamily;
            if (family == null || family.FamilyCategory == null)
            {
                message = "This command can only be run within a family document.";
            ...
            // 2. Find and open the shared parameter file.
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(currentSharedParamFile) || !File.Exists(currentSharedParamFile) || !Path.GetFileName(currentSharedParamFile).Contains(sharedParamFileNameContains))
                {
                    message = "A valid 'RTS_Shared Parameters' file is not configured in Revit. Please set it up via Manage > Shared Parameters and try again.";
                ...
                if (sharedParamFile == null) throw new Exception("Failed to open the shared parameter file. It may be corrupt or inaccessible.");
            }
            catch (Exception ex)
            {
                message = "An error occurred while accessing the shared parameter file: " + ex.Message;
                TaskDialog.Show("Error", message);
                return Result.Failed;
            }

            // 3. Find the 'RTS_Type' parameter definition in the file.
            string paramName = "RTS_Type";
            string paramGuid = "c7e96b3f-aee1-4ecb-b287-6c3a57548acb";
            ExternalDefinition externalDefinition = sharedParamFile.Groups
                .SelectMany(g => g.Definitions)
                .OfType<ExternalDefinition>()
                .FirstOrDefault(d => d.GUID == new Guid(paramGuid));

            if (externalDefinition == null)
            {
                message = $"The required shared parameter '{paramName}' (GUID: {paramGuid}) was not found in '{Path.GetFileName(app.SharedParametersFilename)}'.";
                TaskDialog.Show("Error - Parameter Not Found", message);
                return Result.Failed;
            }

            // 4. Check if the parameter already exists in the family.
            foreach (FamilyParameter fp in doc.FamilyManager.Parameters)
            {
                if (fp.GUID != null && fp.GUID == externalDefinition.GUID)
                {
                    TaskDialog.Show("Information", $"The parameter '{paramName}' already exists in this family.");
                    return Result.Succeeded;
                }
            }

            // 5. Add the parameter to the family.
            Transaction t = new Transaction(doc, "Add RTS_Type Family Parameter");
            try
            {
                t.Start();
                // The third argument 'false' makes it a Type parameter.
                FamilyParameter newParam = doc.FamilyManager.AddParameter(externalDefinition, GroupTypeId.IdentityData, false);
                t.Commit();

                if (newParam != null)
                {
                    TaskDialog.Show("Success", $"The '{newParam.Definition.Name}' Type parameter has been successfully added under 'Identity Data'.");
                    return Result.Succeeded;
                }
                else
                {
                    message = "Failed to add the parameter to the family for an unknown reason.";
                    return Result.Failed;
                }
            }
            catch (Exception ex)
            {
                ...
                message = "An error occurred while adding the parameter: " + ex.Message;
                TaskDialog.Show("Error", message);
                return Result.Failed;
            }
        }
        ...
        /// <summary>
        /// Binds a series of shared parameters to a project document after getting user selections from a UI.
        /// </summary>
        private Result InitiateProjectParameters(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            UIApplication uiapp = commandData.Application;
            ...
            // Display the selection window to the user
            var selectionWindow = new InitiateParametersWindow();
            bool? dialogResult = selectionWindow.ShowDialog();

            // If the user did not click "Initiate", cancel the command
            ...
            {
                new SharedParameterInfo("Cable_01", "cf0d478e-1e98-4e83-ab80-6ee867f61798"),
                new SharedParameterInfo("Cable_02", "2551d308-44ed-405c-8aad-fb78624d086e"),
                new SharedParameterInfo("Cable_03", "c1dfc402-2101-4e53-8f52-f6af64584a9f"),
                new SharedParameterInfo("Cable_04", "f297daa6-a9e0-4dd5-bda3-c628db7c28bd"),
                new SharedParameterInfo("Cable_05", "b0ef396d-6ec0-4ab7-b7cc-9318e9e9b3ab"),
                new SharedParameterInfo("Cable_06", "7c08095a-a3b2-4b78-ba15-dde09a7bc3a9"),
                new SharedParameterInfo("Cable_07", "9bc78bce-0d39-4538-b507-7b98e8a13404"),
                new SharedParameterInfo("Cable_08", "e9d50153-a0e9-4685-bc92-d89f244f7e8e"),
                new SharedParameterInfo("Cable_09", "5713d65a-91df-4d2e-97bf-1c3a10ea5225"),
                new SharedParameterInfo("Cable_10", "64af3105-b2fd-44bc-9ad3-17264049ff62"),
                new SharedParameterInfo("Cable_11", "f3626002-0e62-4b75-93cc-35d0b11dfd67"),
                new SharedParameterInfo("Cable_12", "63dc0a2e-0770-4002-a859-a9d40a2ce023"),
                new SharedParameterInfo("Cable_13", "eb7c4b98-d676-4e2b-a408-e3578b2c0ef2"),
                new SharedParameterInfo("Cable_14", "0e0572e5-c568-42b7-8730-a97433bd9b54"),
                new SharedParameterInfo("Cable_15", "bf9cd3e8-e38f-4250-9daa-c0fc67eca10f"),
                new SharedParameterInfo("Cable_16", "f6d2af67-027e-4b9c-9def-336ebaa87336"),
                new SharedParameterInfo("Cable_17", "f6a4459d-46a1-44c0-8545-ee44e4778854"),
                new SharedParameterInfo("Cable_18", "0d66d2fa-f261-4daa-8041-9eadeefac49a"),
                new SharedParameterInfo("Cable_19", "af483914-c8d2-4ce6-be6e-ab81661e5bf1"),
                new SharedParameterInfo("Cable_20", "c8d2d2fc-c248-483f-8d52-e630eb730cd7"),
                new SharedParameterInfo("Cable_21", "aa41bc4a-e3e7-45b0-81fa-74d3e71ca506"),
                new SharedParameterInfo("Cable_22", "6cffdb25-8270-4b34-8bb4-cf5d0a224dc2"),
                new SharedParameterInfo("Cable_23", "7fdaad3a-454e-47f3-8189-7eda9cb9f6a2"),
                new SharedParameterInfo("Cable_24", "7f745b2b-a537-42d9-8838-7a5521cc7d0c"),
                new SharedParameterInfo("Cable_25", "9a76c2dc-1022-4a54-ab66-5ca625b50365"),
                new SharedParameterInfo("Cable_26", "658e39c4-bbac-4e2e-b649-2f2f5dd05b5e"),
                new SharedParameterInfo("Cable_27", "8ad24640-036b-44d2-af9c-b891f6e64271"),
                new SharedParameterInfo("Cable_28", "c046c4d7-e1fd-4cf7-a99f-14ae96b722be"),
                new SharedParameterInfo("Cable_29", "cdf00587-7e11-4af4-8e54-48586481cf22"),
                new SharedParameterInfo("Cable_30", "a92bb0f9-2781-4971-a3b1-9c47d62b947b"),
                new SharedParameterInfo("Cables On Tray", "c7430aff-c4ee-4354-9601-a060364b43d5"),
                ...
                new SharedParameterInfo("Comment", "f8a844ce-cb1a-4d95-bb11-d48d15a84a8e")
            };
            ...
            try
            {
                ...
                // Attempt to open the shared parameter file.
                try
                {
                    app.SharedParametersFilename = currentSharedParamFile;
                    sharedParamFile = app.OpenSharedParameterFile();
                ...
                // Conditionally process parameter groups based on user selection
                if (selectionWindow.InitiateDetailItems)
                {
                ...
                if (selectionWindow.InitiateCableConduit)
                {
                ...
                if (selectionWindow.InitiateRtsId)
                {
                    summaryMessage.AppendLine("\nStep 4: Processing RTS_ID Parameter...");
                ...
                if (selectionWindow.InitiateWires)
                {
            ...
            catch (Exception ex)
            {
                ...
                TaskDialog.Show("RTS Initiate Parameters - Critical Error", summaryMessage.ToString());
                return Result.Failed;
            }
        }
        ...
        /// <summary>
        /// Processes a list of shared parameters, binding them to the specified categories in a project.
        /// </summary>
                                       ...
                                       string categorySetNameForMessages)
        {
            ...
            bool isDetailComponentCategory = false;
            Category detailCat = doc.Settings.Categories.get_Item(BuiltInCategory.OST_DetailComponents);
            if (detailCat != null && targetCategories.Contains(detailCat) && targetCategories.Size == 1)
            {
                isDetailComponentCategory = true;
            }
            ...
            foreach (SharedParameterInfo paramInfo in parametersToProcess)
            {
                ...
                try
                {
                    ...
                    if (externalDefinition == null)
                    {
                        summaryMessage.AppendLine($"   -> ERROR: Definition not found in shared parameter file for GUID: {paramInfo.GuidString}.");
                        continue;
                    }
                        ...
                        if (spElement != null)
                        {
                            ...
                            if (internalDef != null)
                            {
                                ...
                                if (!isDetailComponentCategory)
                                {
                                    ...
                                    {
                                        summaryMessage.AppendLine($"     -> WARNING: Could not set 'Vary by Group' for '{paramInfo.Name}'. (Error: {exSetVary.Message})");
                                    }
                                }
                                else
                                {
                                    summaryMessage.AppendLine("     -> INFO: Skipping 'Vary by Group' for Detail Item category.");
                                }
                            }
                        }
                    }
        ...
        /// <summary>
        /// Creates a detailed log of an exception.
        /// </summary>
