            ...
            ElementSet elements)
        {
            ...
            using (Transaction t = new Transaction(doc, "Update RTS_IDs and Cable Parameters")) // Changed transaction name
            {
                try
                {
                    ...
                    t.Commit();
                    CustomTaskDialog.Show("RT_TrayID", $"{updatedCableTrayCount} Cable Trays and {otherElementsUpdatedCount} other elements (Detail Items, Fittings, Conduits) updated successfully.");
                }
                catch (Exception ex)
                {
                    t.RollBack();
                    message = $"Error: {ex.Message}";
                    CustomTaskDialog.Show("RT_TrayID Error", message);
