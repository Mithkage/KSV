    ...
    public class RT_TrayOccupancyClass : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            try
            {
                ...
                {
                    CustomTaskDialog.Show("No Data Found", "No valid cable data was found in the project's primary extensible storage. Please run the 'PC_Extensible' command to import data first. The process will now exit.");
                ...
                if (cableLengths.Any())
                {
                    ...
                    using (Transaction tx = new Transaction(doc, "Save Model Generated Data"))
                    {
                        ...
                        try
                        {
                            ...
                            tx.Commit();
                            CustomTaskDialog.Show("Model Data Saved", $"Calculated Cable Lengths data successfully saved/merged into Model Generated Data storage.\nUpdated entries: {updatedEntriesCount}\nAdded entries: {addedEntriesCount}");
                        }
                        catch (Exception ex)
                        {
                            tx.RollBack();
                            CustomTaskDialog.Show("Model Data Save Error", $"Failed to save Model Generated Data: {ex.Message}");
                        }
                    }
                }

                // --- 8. NOTIFY USER OF OVERALL SUCCESS ---
                CustomTaskDialog.Show("Process Complete", $"Process complete. Revit model has been updated and files have been saved to:\n{outputFolderPath}");
