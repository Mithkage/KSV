...
using System.Linq;
using System.Text;
using System.Windows.Forms; // For CustomTaskDialog, OpenFileDialog removed for core function
using System.Text.Json;
using RTS.Commands.Support;
#endregion

namespace RTS.Commands.PowerReticulation
{
    /// <summary>
    ...
    public class RT_PanelConnectClass : IExternalCommand
    {
            ...
            ElementSet elements)
        {
            ...
            try
            {
                ...
                {
                    CustomTaskDialog.Show("No Data Found", "No cleaned cable data (PC_Data) was found in the project's extensible storage. Please run 'Process & Save Cable Data' first.");
                ...
                {
                    CustomTaskDialog.Show("No Valid Connections", "No valid (From, To) connection pairs were found in the stored PC_Data after filtering. Process cancelled.");
                    return Result.Cancelled;
                }
            ...
            CustomTaskDialog.Show("Process Report", summaryMessage);
        ...
        private List<CableData> RecallCableDataFromExtensibleStorage(Document doc)
        {
            ...
            {
                CustomTaskDialog.Show("Data Recall Error", $"Failed to deserialize stored PC_Data: {ex.Message}. The stored data might be corrupt or incompatible.");
                return new List<CableData>();
            }
            catch (Exception ex)
            {
                CustomTaskDialog.Show("Data Recall Error", $"An unexpected error occurred during PC_Data recall: {ex.Message}");
                return new List<CableData>();
            }
        }
        #endregion
