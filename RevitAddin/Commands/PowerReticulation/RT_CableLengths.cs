...
using Autodesk.Revit.UI;                 // For UI elements like CustomTaskDialog
...
namespace RTS.Commands.PowerReticulation // Namespace to organize the code, reflecting the file name
{
    /// <summary>
    ...
    public class RTCableLengthsCommand : IExternalCommand
    {
            ...
            ElementSet elements)
        {
            ...
            try
            {
                ...
                if (!targetItems.Any())
                {
                    CustomTaskDialog.Show("Info", "No Detail Items or Generic Annotations found in the project.");
                ...
                {
                    CustomTaskDialog.Show("Info", "No target items found with a value in their lookup parameter.");
                    return Result.Succeeded;
                }
                ...
                if (step5UpdateSuccessful)
                {
                    ...
                    {
                        CustomTaskDialog.Show("Success", "Cable lengths updated successfully.");
                    }
                    else if (targetItemMapByLookupValue.Any())
                    {
                        CustomTaskDialog.Show("Info", "No matching Conduits/Cable Trays found with lengths, or their 'Cable_XX' parameter was empty/did not match. 'PC_Cable_Length' parameters remain cleared or as they were.");
                    }
                    else
                    {
                        CustomTaskDialog.Show("Info", "No operations performed. Check if target items and relevant Conduits/Cable Trays exist with matching lookup values.");
                    }
                    return Result.Succeeded;
                }
                else
                {
                    ...
                    CustomTaskDialog.Show("Failed", message);
            ...
            catch (Exception ex)
            {
                ...
                CustomTaskDialog.Show("Critical Error", message);
                return Result.Failed;
            }
        }
    }
