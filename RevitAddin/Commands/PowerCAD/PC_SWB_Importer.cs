...
namespace RTS.Commands.PowerCAD
{
    ...
    public class PC_SWB_ImporterClass : IExternalCommand
    {
          ...
          ElementSet elements)
        {
            ...
            try
            {
                using (TextFieldParser parser = new TextFieldParser(csvFilePath, Encoding.Default)) // Consider Encoding.UTF8 for wider compatibility
                {
                    ...
                    if (!parser.EndOfData)
                    {
                        ...
                        if (headers == null || headers.Length == 0) { message = "CSV header row missing or empty."; CustomTaskDialog.Show("CSV Error", message); return Result.Failed; }
                        swbToCsvIndex = Array.FindIndex(headers, h => h.Equals("SWB To", StringComparison.OrdinalIgnoreCase));
                        if (swbToCsvIndex < 0) { message = "CSV header row does not contain the required 'SWB To' column."; CustomTaskDialog.Show("CSV Error", message); return Result.Failed; }
                    }
                    else { message = "CSV file appears empty (no header row found)."; CustomTaskDialog.Show("CSV Error", message); return Result.Failed; }
                ...
                if (!csvData.Any()) { message = "No valid data rows found in CSV. Check that the 'SWB To' column contains values."; CustomTaskDialog.Show("CSV Warning", message); return Result.Succeeded; }
            ...
            catch (IOException ioEx) { message = $"Error accessing CSV file: {ioEx.Message}\nEnsure the file is not open in another program."; CustomTaskDialog.Show("File Access Error", message); return Result.Failed; }
            catch (Exception ex) { long lineNumber = ex is MalformedLineException mex ? mex.LineNumber : -1; message = $"Error reading/parsing CSV" + (lineNumber > 0 ? $" near line {lineNumber}" : "") + $": {ex.Message}"; CustomTaskDialog.Show("CSV Error", message); return Result.Failed; }
            ...
            catch (Exception ex) { message = $"Error filtering elements: {ex.Message}"; CustomTaskDialog.Show("Revit Element Error", message); return Result.Failed; }

            if (!detailItemsToUpdate.Any()) { CustomTaskDialog.Show("Info", "No Detail Items found matching criteria (PC_PowerCAD=Yes and non-empty PC_SWB To)."); return Result.Succeeded; }
            ...
            using (Transaction tx = new Transaction(doc))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    CustomTaskDialog.Show("Transaction Error", message); return Result.Failed;
                }
            ...
            if (errors.Any())
            {
                ...
                CustomTaskDialog.Show("Import Report with Errors/Warnings", summary);
            }
            else { CustomTaskDialog.Show("Import Report", summary); }
