...
//
// Log:
// 2025-07-09:   Refactored all dialogs to use the enhanced, self-contained CustomTaskDialog.
// 2025-07-09:   Corrected all TaskDialog calls to use the native Autodesk.Revit.UI.TaskDialog to resolve ambiguity and feature-related errors.
// 2025-07-08:   Corrected the ClearData method to properly delete the DataStorage element, which is the supported method in the Revit API.
// 2025-07-08:   Replaced CustomTaskDialog-based UI with a modern WPF window for a better user experience.
// 2025-07-08:   Refactored Execute method to launch WPF window and process results.
// 2025-07-08:   Implemented session memory for the last browsed folder path.
// 2025-07-08:   Added "Load (A)" to the CSV import process and data model.
// 2025-07-07:   Corrected VendorId to match the .addin manifest file and resolve runtime error.
//
#region Namespaces
using System;
...
namespace RTS.Commands.PowerCAD
{
    ...
    public class PC_ExtensibleClass : IExternalCommand
    {
        ...
        private Result HandleClearData(Document doc, ref string message)
        {
            ...
            CustomTaskDialog clearDialog = new CustomTaskDialog("Clear Data Options");
            clearDialog.MainContent = "Which data set would you like to clear?";
            clearDialog.AddCommandLink(CustomTaskDialogCommandLinkId.CommandLink1, "Clear My PowerCAD Data Only");
            clearDialog.AddCommandLink(CustomTaskDialogCommandLinkId.CommandLink2, "Clear Consultant PowerCAD Data Only");
            clearDialog.AddCommandLink(CustomTaskDialogCommandLinkId.CommandLink4, "Clear Model Generated Data Only");
            clearDialog.AddCommandLink(CustomTaskDialogCommandLinkId.CommandLink3, "Clear All Data (My, Consultant, and Model Generated)");
            clearDialog.CommonButtons = CustomTaskDialogCommonButtons.Cancel;
            clearDialog.DefaultButton = CustomTaskDialogResult.CommandLink1;

            CustomTaskDialogResult clearResult = clearDialog.Show();

            if (clearResult == CustomTaskDialogResult.CommandLink1)
            {
                return ClearData(doc, PrimarySchemaGuid, PrimaryDataStorageElementName, "My PowerCAD Data");
            }
            else if (clearResult == CustomTaskDialogResult.CommandLink2)
            {
                return ClearData(doc, ConsultantSchemaGuid, ConsultantDataStorageElementName, "Consultant PowerCAD Data");
            }
            else if (clearResult == CustomTaskDialogResult.CommandLink4)
            {
                return ClearData(doc, ModelGeneratedSchemaGuid, ModelGeneratedDataStorageElementName, "Model Generated Data");
            }
            else if (clearResult == CustomTaskDialogResult.CommandLink3)
            {
                ...
                {
                    CustomTaskDialog.Show("Clear Data Complete", "All selected PowerCAD data stores were processed successfully.");
            ...
            Func<string, List<string>, List<T>> csvParser) where T : class, new() // Constraint for T
        {
            ...
            try
            {
                ...
                if (!csvFiles.Any())
                {
                    CustomTaskDialog.Show("No CSVs Found", $"No CSV files were found in the selected folder: {sourceFolderPath}. No data will be imported.");
                    return Result.Succeeded;
                }
                ...
                {
                    CustomTaskDialog.Show("No Relevant Data", $"No relevant 'Cables Summary' data found across all CSV files in '{sourceFolderPath}'. No changes will be made to project data.");
                    return Result.Succeeded;
                }
                ...
                using (Transaction tx = new Transaction(doc, $"Save Merged {dataTypeName} to Extensible Storage"))
                {
                    ...
                    try
                    {
                        ...
                        CustomTaskDialog.Show("Extensible Storage Save", summaryMessage);
                    }
                    catch (Exception ex)
                    {
                        tx.RollBack();
                        ...
                        CustomTaskDialog.Show("Extensible Storage Error", msg);
                        return Result.Failed;
                    }
                }
                ...
                CustomTaskDialog.Show("Bulk Import Summary", finalSummary.ToString());
            ...
            catch (Exception ex)
            {
                ...
                CustomTaskDialog.Show("Error", msg);
                return Result.Failed;
            }
        }
        ...
        {
            CustomTaskDialog confirmDialog = new CustomTaskDialog("Confirm Clear Data");
            confirmDialog.MainContent = $"Are you sure you want to clear ALL existing '{dataTypeName}' data from the project?\n\nThis action cannot be undone.";
            confirmDialog.CommonButtons = CustomTaskDialogCommonButtons.Yes | CustomTaskDialogCommonButtons.No;
            confirmDialog.DefaultButton = CustomTaskDialogResult.No;

            if (confirmDialog.Show() == CustomTaskDialogResult.Yes)
            {
                using (Transaction tx = new Transaction(doc, $"Clear {dataTypeName} from Extensible Storage"))
                {
                    ...
                    try
                    {
                        ...
                        if (dataStorageToDelete != null)
                        {
                            ...
                            tx.Commit();
                            CustomTaskDialog.Show("Clear Data Complete", $"The DataStorage element for '{dataTypeName}' was successfully deleted.");
                            return Result.Succeeded;
                        }
                        else
                        {
                            tx.RollBack();
                            CustomTaskDialog.Show("Clear Data Info", $"No existing '{dataTypeName}' data was found in the project to clear.");
                            return Result.Succeeded;
                        }
                    }
                    catch (Exception ex)
                    {
                        ...
                        CustomTaskDialog.Show("Clear Data Error", msg);
                        return Result.Failed;
                    }
                }
            }
            ...
            {
                CustomTaskDialog.Show("Clear Data Cancelled", $"Clear '{dataTypeName}' operation cancelled.");
                return Result.Cancelled;
            }
        }
            ...
            string dataStorageElementName)
        {
            ...
            {
                CustomTaskDialog.Show("Data Recall Error", $"Failed to deserialize stored {schemaName} data: {ex.Message}. The stored data might be corrupt or incompatible.");
                return new List<T>();
            }
            catch (Exception ex)
            {
                CustomTaskDialog.Show("Data Recall Error", $"An unexpected error occurred during {schemaName} data recall: {ex.Message}");
                return new List<T>();
            }
        }
        ...
        {
            CustomTaskDialog.Show("Information", "This command does not directly import 'Model Generated Data' via CSV. It is managed by other scripts.");
