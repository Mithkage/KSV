...
using ClosedXML.Excel;     // Required for ClosedXML
using System.IO;            // Required for Path operations

namespace MD_Importer
{
    ...
    public class MD_ImporterClass : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            try
            {
                using (XLWorkbook workbook = new XLWorkbook(filePath))
                {
                    ...
                    catch (Exception ex) // Catch other potential errors like corrupted file
                    {
                        ...
                        TaskDialog.Show("Excel Error", message);
                        return Result.Failed;
                    }
                    ...
                    if (table == null)
                    {
                        ...
                        TaskDialog.Show("Table Not Found", message);
                        return Result.Failed;
                    }
                    ...
                    if (!table.Fields.Any(f => f.Name.Equals(ExcelKeyColumnName, StringComparison.OrdinalIgnoreCase)))
                    {
                        ...
                        TaskDialog.Show("Column Not Found", message);
                        return Result.Failed;
                    }
                ...
                if (excelDataMap.Count == 0)
                {
                    ...
                    TaskDialog.Show("No Data Extracted", message);
            ...
            catch (Exception ex)
            {
                ...
                TaskDialog.Show("Excel Read Error", message);
                return Result.Failed;
            }
            ...
            if (!detailItems.Any())
            {
                TaskDialog.Show("Info", "No Detail Items found in the current Revit document.");
            ...
            using (Transaction trans = new Transaction(doc, "Update Detail Item PC_SWB Load from Excel"))
            {
                if (trans.Start() == TransactionStatus.Started)
                {
                    ...
                    if (trans.Commit() == TransactionStatus.Committed)
                    {
                        ...
                        TaskDialog.Show("Update Complete", summary);
                    ...
                    else
                    {
                        ...
                        TaskDialog.Show("Transaction Error", message);
                        return Result.Failed;
                    }
                }
                else
