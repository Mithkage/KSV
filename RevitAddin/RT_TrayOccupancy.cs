    ...
    public class RT_TrayOccupancyClass : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            try
            {
                ...
                // We instantiate PC_ExtensibleClass to access its public RecallCableDataFromExtensibleStorage method.
                PC_ExtensibleClass pcExtensible = new PC_ExtensibleClass();
                List<PC_ExtensibleClass.CableData> cleanedData = pcExtensible.RecallCableDataFromExtensibleStorage(doc);

                if (cleanedData == null || cleanedData.Count == 0)
                {
                    TaskDialog.Show("No Data Found", "No valid cable data was found in the project's extensible storage. Please run the 'PC_Extensible' command to import data first. The process will now exit.");
        ...
        private void ExportDataToCsv(List<PC_ExtensibleClass.CableData> data, string filePath)
        {
            ...
            // Headers must match the properties of PC_ExtensibleClass.CableData
        ...
        private List<TrayCableData> CollectTrayData(Document doc, List<PC_ExtensibleClass.CableData> cableScheduleData)
        {
            ...
            foreach (var entry in groupedCables)
            {
                ...
                foreach (string cableRef in sortedCables)
                {
                    ...
                    // This assumes CableReference is unique enough for lookup.
        ...
        private List<CableLengthData> CollectCableLengthsData(Document doc, List<PC_ExtensibleClass.CableData> cableScheduleData)
        {
            ...
            foreach (var kvp in cableRunData)
            {
                ...
                // Lookup using the 'To' property, as it's the key in cableDataDict
                // Note: The cableRef here comes from the Revit element's Cable_XX parameter,
                // which is a 'Cable Reference'. We need to find the CableData entry
                // from the stored data that corresponds to this Cable Reference.
                // The previous logic of using .ToDictionary(c => c.CableReference, c => c)
                // was problematic if CableReference was not unique or empty.
                // Now, we iterate through cableScheduleData to find the matching entry.
                var cableInfo = cableScheduleData.FirstOrDefault(c => c.CableReference == cableRef);
