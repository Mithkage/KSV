...
using RTS.Commands.PowerCAD; // For WindowInteropHelper
...
namespace RTS_Reports
{
    ...
    public partial class ReportSelectionWindow : Window
    {
        ...
        private void GenerateRSGxCableScheduleCsvReport_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                CustomTaskDialog.Show("Export Cancelled", "Output folder not selected. RSGx Cable Schedule export cancelled.");
                return;
            }
            ...
            try
            {
                ...
                if (!reportData.Any())
                {
                    CustomTaskDialog.Show("No Data", "No unique cable references found to generate the RSGx Cable Schedule.");
                }

                ExportDataToCsvGeneric(reportData, filePath, "RSGx Cable Schedule");
                CustomTaskDialog.Show("Export Complete", $"RSGx Cable Schedule successfully exported to:\n{filePath}");
            }
            catch (Exception ex)
            {
                CustomTaskDialog.Show("Export Error", $"Failed to export RSGx Cable Schedule: {ex.Message}");
            }
        }
        ...
        private void PerformExport<T>(Guid schemaGuid, string schemaName, string fieldName, string dataStorageElementName, string defaultFileName, string dataTypeName) where T : class, new()
        {
            ...
            {
                CustomTaskDialog.Show("No Data", $"No {dataTypeName} found to export.");
                return;
            }
            ...
            if (string.IsNullOrEmpty(outputFolderPath))
            {
                CustomTaskDialog.Show("Export Cancelled", "Output folder not selected.");
                return;
            }
            ...
            try
            {
                ...
                CustomTaskDialog.Show("Export Complete", $"{dataTypeName} successfully exported to:\n{filePath}");
            }
            catch (Exception ex)
            {
                CustomTaskDialog.Show("Export Error", $"Failed to export {dataTypeName}: {ex.Message}");
            }
        }
