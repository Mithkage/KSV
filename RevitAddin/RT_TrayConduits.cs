...
// Removed: using static Autodesk.Revit.DB.Units.UnitUtils; // Removed to use manual conversion



namespace RT_TrayConduits

{

    // Ensure these attributes are applied only once to the class
...
            // --- Workset setup removed. Conduits will be placed in the active workset. ---
...
                        XYZ trayNormal = Autodesk.Revit.DB.XYZ.BasisZ.CrossProduct(trayDirection).Normalize();
...
                                        // The first conduit is at (trayWidth / 2) - conduitRadius from the center (right edge aligned with conduit center)

                                        // Subsequent conduits move inwards by 2 * conduitDiameter

                                        double currentOffsetFromCenterline = (trayWidthFeet / 2.0) - conduitRadiusFeet - (conduitCountOnTray * 2 * desiredConduitDiameterFeet);
